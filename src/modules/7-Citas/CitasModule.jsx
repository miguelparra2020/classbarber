...
const CitasModule = () => {
    ...
    const [loadingCitasCel, setLoadingCitasCel] = useState(false)
    ...
    const calcularHorariosDisponibles = (horariosOcupados, duracionFranja = 30) => {
      ...
      const fechaSeleccionada = new Date(selectedFecha.year, selectedFecha.monthNumber - 1, selectedFecha.day);

      // Si es el día actual, ajustar el inicio del día a la hora actual
      if (fechaActual.getTime() === fechaSeleccionada.getTime()) {
        inicioDia.setHours(ahora.getHours(), ahora.getMinutes(), 0);
      }
      ...
      // Filtrar las franjas que no se solapan con los horarios ocupados
    ...
    const bodyToCreateCita = {
      ...
      `,
      "start": {
        "dateTime": `${selectedFecha?.year}-${selectedFecha?.monthNumber}-${selectedFecha?.day}T${sumarMinutos(`${selectHoraDisponible?.start}`, 0).slice(0, 5)}:00`,
        "timeZone": "Europe/Madrid"
      },
      "end": {  
        "dateTime": `${selectedFecha?.year}-${selectedFecha?.monthNumber}-${selectedFecha?.day}T${sumarMinutos(`${selectHoraDisponible?.start}`, arrayServicios[selectedServicio - 1]?.minutos).slice(0, 5)}:00`,
        "timeZone": "Europe/Madrid"
      },
    }
                                ...
                                <li className="ms-8">
                                    ...
                                    <h3 className="mb-1 text-lg font-semibold text-gray-900 ">
                                      ...
                                      <div>
                                          ...
                                          className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 " placeholder="Nombre usuario" required />
                                            </div>
                                      <div>
                                          <label for="email" className="block mb-2 text-sm font-medium text-gray-900 ">
                                            Correo electrónico:</label>
                                          <input type="email" value={emailCustomer} name="email" id="email" onChange={(e) => setEmailCustomer(e.target.value)}
                                          className="bg-gray-50 border mb-2 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 " placeholder="name@gmail.com" required />
                                        </div>
                                      <div>
                                          <label for="email" className="block mb-2 text-sm font-medium text-gray-900 ">
                                            Celular contacto:</label>
                                          <input type="number" value={celCustomer} name="number" id="number" onChange={(e) => setCelCustomer(e.target.value)}
                                          className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 " placeholder="613670695" required />
                                    </div>
  ...
  <path d="M8 14a1 1 0 1 0 0-2 1 1 0 0 0 0 2"/>
    ...
    <button onClick={() => handleSearhCita(celCustomerToSearh)} disabled={loadingCitasCel}
    className="text-white inline-flex w-[35%] justify-center bg-gray-500 
    hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium 
